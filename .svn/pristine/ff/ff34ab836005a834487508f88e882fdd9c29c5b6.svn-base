    ...
    private function crumb_to_link( $link, $i ) {
        ...
        if ( isset( $link['text'] ) && ( is_string( $link['text'] ) && $link['text'] !== '' ) ) {
            ...
            if ( ( isset( $link['url'] ) && ( is_string( $link['url'] ) && $link['url'] !== '' ) ) &&
                ...
                if ( $i === 0 ) {
                    $link_output .= '<' . $this->list . ' itemscope itemtype="http://schema.org/BreadcrumbList">';
                }

                $title_attr   = isset( $link['title'] ) ? ' title="' . esc_attr( $link['title'] ) . '"' : '';
                $item = '<li class="item-breadcrumbs" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                        <a class="link-breadcrumbs" itemprop="item" href="' . esc_url( $link['url'] ) . '">
                            <span class="text-breadcrumbs" itemprop="name">' . $link['text'] . '</span>
                        </a><meta itemprop="position" content="' . $i . '" />
                      </li>';
                $link_output .= $item;
            }
            else {
                $last_item = '<li class="item-breadcrumbs last-breadcrumbs" itemprop="itemListElement" itemscope itemtype="http://schema.org/ListItem">
                        <span class="text-breadcrumbs" itemprop="name">' . $link['text'] . '</span>
                        <meta itemprop="position" content="' . $i . '" />
                      </li>';
                $link_output .= $last_item;
