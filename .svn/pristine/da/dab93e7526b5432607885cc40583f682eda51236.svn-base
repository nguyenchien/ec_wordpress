...
function dynimg_404_handler() {
    ...
    if (preg_match('/(.*)-([0-9]+)x([0-9]+)(c)?\.(jpg|png|gif|jpeg)/ui',$_SERVER['REQUEST_URI'],$matches)) {
...
function swe_wp_get_attachment_image_src($attachment_id, $size='thumbnail', $icon = false) {
    ...
    if(checkPostIDInSiteMedia($attachment_id)){
        ...
        if (is_array($size)){
            ...
            if($arr && $size){
                $img_src = $arr[0];
                if (preg_match('/(.*)\.(jpg|png|gif|jpeg)/i',$img_src,$matches)) {
        ...
        $arr[0] = preg_replace( '/\/sites\/[0-9]{1,2}/i', '', $arr[0]);
        //$arr[0] =  str_replace(get_site_url($sites['blog_media']).'/', '', $arr[0]);
...
function swe_wp_get_attachment_image($attachment_id, $size = 'thumbnail', $icon = false, $attr = ''){
    ...
    if ( $image ) {
        ...
        $default_attr = array(
            ...
            'alt'	=> getTranslateContent(trim(strip_tags( get_post_meta($attachment_id, '_wp_attachment_image_alt', true) ))), // Use Alt field first
        );
        if ( empty($default_attr['alt'])  && !empty($attachment->post_excerpt))
            $default_attr['alt'] = getTranslateContent(trim(strip_tags( $attachment->post_excerpt ))); // If not, Use the Caption
        if ( empty($default_attr['alt'])  && !empty($attachment->post_title))
            $default_attr['alt'] = getTranslateContent(trim(strip_tags( $attachment->post_title ))); // Finally, use the title
